...
public abstract class BaseRedis<K, V> {
     ...
     */
    public Long dbSize() {
        Long result = transRedisTemplate.execute((RedisCallback<Long>) connection -> connection.dbSize());
        return result;
    }

    public void clean(K pattern) {
        Set<K> keySet = redisTemplate.keys(pattern);
    ...
    private RedisTemplate<K, V> getRedisTemplate(Boolean suppTrans){
        ...
        transRedisTemplate.setEnableTransactionSupport(true);
        return suppTrans ? redisTemplate : transRedisTemplate ;
    }
